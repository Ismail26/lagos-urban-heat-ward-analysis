...
- **Landsat imagery and GeoTIFF derivatives:** excluded because the original project contains tens of gigabytes of raster data. Original Landsat scenes should be obtained from USGS EarthExplorer; derived inputs can be regenerated following the manuscript methods.
...
Figures based only on the four derived workbooks can be regenerated directly after installing the Python dependencies. LST and LULC raster panels require the large inputs documented in `data/raw/README.md`. Corridor composites also require the separately licensed satellite-image panels. The final versions of all figures are retained in `figures/` so the archived release records the exact publication outputs.
